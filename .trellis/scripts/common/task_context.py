...
def _to_repo_relative(path: Path, repo_root: Path) -> str:
    """Convert a path to repo-relative POSIX style when possible."""
    try:
...
def _pick_existing_path(repo_root: Path, candidates: list[str], fallback: str) -> str:
    """Pick the first existing file path from candidates."""
...
def get_check_context(repo_root: Path) -> list[dict]:
    ...
    finish_work_path = adapter.get_trellis_command_path("finish-work")
    check_command_path = adapter.get_trellis_command_path("check")
    platform_skill_path = f"{adapter.config_dir_name}/skills/trellis-check/SKILL.md"
    agent_check_path = _to_repo_relative(
        adapter.get_agent_path("trellis-check", repo_root),
        repo_root,
    )
    check_path = _pick_existing_path(
        repo_root,
        [
            check_command_path,
    ...
    return [
        {"file": finish_work_path, "reason": "Finish work checklist"},
        {"file": check_path, "reason": "Code quality check spec"},
    ]
